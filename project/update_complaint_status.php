...
else if($_POST[$i]=="On Hold")
{
mysql_query("UPDATE complaint_images_updated SET status_code='2' WHERE imagesrc='$row[imagesrc]'");
mysql_query("UPDATE complaint_images_updated SET status='On Hold' WHERE imagesrc='$row[imagesrc]'");
}
else
{
mysql_query("UPDATE complaint_images_updated SET status_code='3' WHERE imagesrc='$row[imagesrc]'");
mysql_query("UPDATE complaint_images_updated SET status='Invalid' WHERE imagesrc='$row[imagesrc]'");
}
}
